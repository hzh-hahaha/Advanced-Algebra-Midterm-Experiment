...
并使用 PSNR 和 SSIM 作为图像质量辅助指标。需要注意的是，PSNR 和 SSIM 用于描述视觉质量，而矩阵范数误差更直接反映线性代数意义下的恢复精度。
...
3. Tikhonov 正则化通过引入 $\lambda\|x\|_2^2$ 限制解范数，并在 SVD 基底下形成谱滤波因子 $\sigma_i/(\sigma_i^2+\lambda)$；
